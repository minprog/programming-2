...
* `INSERT` the new user into `users`, storing a hash of the user's password, not the password itself. Odds are you'll find http://pythonhosted.org/passlib/lib/passlib.context.html?#passlib.context.CryptContext.hash[pwd_context.encrypt] of interest.
